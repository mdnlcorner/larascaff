...
btnToggle.forEach(function(item) {
    if (item.dataset.themeMode == 'dark' && localStorage.getItem('color-theme') === 'dark') {
        ...
        item.classList.add('text-primary')
    } else if (item.dataset.themeMode == 'system' && (localStorage.getItem('color-theme') === 'system') || (!('color-theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
        item.classList.add('text-primary')
    }
