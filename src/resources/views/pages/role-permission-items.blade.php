        ...
        <td>
            <div class="flex items-center gap-2">
                @foreach ($mm->permissions as $permission)
                    <x-larascaff::forms.checkbox label="{{ explode(' ',$permission->name)[0] }}" :checked="$data->hasPermissionTo($permission->name)" class="child"  name="permissions[]" value="{{ $permission->name }}" id="permission-{{ $mm->id.'-'.$permission->id }}" />
                @endforeach
            </div>
        </td>
    </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @foreach ($ssm->permissions as $smp)
                                        <x-larascaff::forms.checkbox label="{{ explode(' ',$smp->name)[0] }}" :checked="$data->hasPermissionTo($smp->name)" class="child"  name="permissions[]" value="{{ $smp->name }}" id="permission-{{ $sm->id.'-'.$smp->id }}" />
                                    @endforeach
                                    </div>
                                </td>
                            </tr>
