...
class Dropdown implements DropdownInterface {
    ...
    constructor(
        ...
        this._instanceId = instanceOptions.id ? instanceOptions.id : (targetElement.dataset.dropdownMenu ?? '');
    ...
    destroy() {
        ...
        if (this._options.triggerType === 'hover') {
            triggerEvents.showEvents.forEach((ev) => {
                this._triggerEl.removeEventListener(ev, this._hoverShowTriggerElHandler);
                this._targetEl.removeEventListener(ev, this._hoverShowTargetElHandler);
            });
    ...
    _setupEventListeners() {
        ...
        if (this._options.triggerType === 'hover') {
            triggerEvents.showEvents.forEach((ev) => {
                this._triggerEl.addEventListener(ev, this._hoverShowTriggerElHandler);
                this._targetEl.addEventListener(ev, this._hoverShowTargetElHandler);
            });
    ...
    _handleClickOutside(ev: Event, targetEl: HTMLElement) {
        ...
        if (ignoreClickOutsideClass) {
            const ignoredClickOutsideEls = document.querySelectorAll(`.${ignoreClickOutsideClass}`);
...
export function initDropdowns(dropdown: HTMLElement) {
    if (dropdown) {
        ...
        if ($dropdownEl && !instance) {
            const placement = dropdown.getAttribute('data-dropdown-placement');
            const offsetSkidding = dropdown.getAttribute('data-dropdown-offset-skidding');
            const offsetDistance = dropdown.getAttribute('data-dropdown-offset-distance');
            const triggerType = dropdown.getAttribute('data-dropdown-trigger');
            const delay = dropdown.getAttribute('data-dropdown-delay');
            const ignoreClickOutsideClass = dropdown.getAttribute('data-dropdown-ignore-click-outside-class');
